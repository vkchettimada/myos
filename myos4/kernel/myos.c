...
static context_t myOsContext[MYOS_MAX_TASK];

extern void (*i8253_hook)(void);

static void myOsIdle(void)
{
	while(1) {
		MYOS_DEBUG("idle ");
		myOsDelay(1);
...
static void myOsTimer(void)
{
	...
	for (i=0;i<myOsTasks;i++) {
		if (myOsContext[i].sleep>0)
			myOsContext[i].sleep--;
		else if (i == myOsCurrTask && myOsContext[i].count>0)
			myOsContext[i].count--;
	}
}

static int myOsEligible()
{
...
{
	MYOS_DEBUG("myOsInit\n");
	myOsStackEnd=MYOS_SP_START;
	myOsTasks=myOsCurrTask=0;
	i8253_hook=myOsTimer;
	myOsAddTask(myOsIdle, 4096, 0);
}
...
int myOsAddTask(MyOsTask taskAddr, int stackSize, int priority)
{
	...
	if (myOsTasks<MYOS_MAX_TASK)
	{
		...
		myOsStackEnd+=(stackSize*MYOS_SP_DIR);
...
void myOsDelay(int sleep)
{
	...
	MYOS_DEBUG("myOsDelay\n");
	myOsContext[myOsCurrTask].sleep=sleep;
	myOsSwitch(myOsEligible());
}

void myOsSwitch(int eligible)
{
	int BP,AX,BX,CX,DX,DI,SI;
	...
	myOsContext[myOsCurrTask].DI=DI;
