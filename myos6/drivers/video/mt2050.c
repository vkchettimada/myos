...
static void mt2050_set_if_freq(struct i2c_client *c,unsigned int freq, unsigned int if2)
{
	...
	dprintk(KERN_DEBUG"mt2050_set_if_freq freq=%d\n",freq);
	...
	dprintk(KERN_DEBUG"lo1 lo2 = %d %d\n", lo1, lo2);
        dprintk(KERN_DEBUG"num1 num2 div1a div1b div2a div2b= %x %x %x %x %x %x\n",num1,num2,div1a,div1b,div2a,div2b);
	...
	dprintk(KERN_DEBUG"mt2050: buf= %x %x %x %x %x %x\n",buf[0], buf[1], buf[2], buf[3], buf[4], buf[5]);
	
        if (ret!=6)
                dprintk("mt2050_set_if_freq failed with %d\n",ret);
}
*/
...
/*
static int mt2050_deinit(struct i2c_client *c)
{
	unsigned char buf[2];
	int ret;
	...
	buf[0]=6;
	buf[1]=0x00;
	ret=i2c_master_send(c,buf,2); //  power off, bit4=0
	
	dprintk(KERN_DEBUG"mt2050: deinit called\n");
	return 0;
}
