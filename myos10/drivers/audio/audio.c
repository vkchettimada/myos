...
struct ac97_inst_t {
	...
	struct ac97_bd_t mic_in_list[AC97_BD_MAX];
	struct ac97_bd_t pcm_in_list[AC97_BD_MAX];
...
{
	int index;
	struct ac97_bd_t *mic_in_list, *pcm_in_list, *pcm_out_list;

	mic_in_list = ac97_inst->mic_in_list;
	pcm_in_list = ac97_inst->pcm_in_list;
	pcm_out_list = ac97_inst->pcm_out_list;

	for (index=0; index <AC97_BD_MAX; index++) {
		mic_in_list[index].buffer_len = 0;
		mic_in_list[index].IOC = 1;
		mic_in_list[index].BUP = 1;
		pcm_in_list[index].buffer_len = 0;
...
int audio_play(short *data, int *len)
{
	...
	if (*len > AC97_BUFFER_LEN)
		*len = AC97_BUFFER_LEN;
	memcpy((void*)ac97_inst->pcm_out_list[lvi].buffer_ptr, data, *len);
	ac97_inst->pcm_out_list[lvi].buffer_len = *len/2;
