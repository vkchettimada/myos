...
#define  PCI_REG_CMD_MASTER		0x0004
...
int pci_get_data(struct pci_dev_t *pci_dev
		, unsigned char reg
		, unsigned len
		, unsigned char *data)
{
	switch(len)
	{
		case 1:
			{
				g_pci_read_config_byte(pci_dev, reg, data);
			}
			break;
		case 2:
			{
				g_pci_read_config_word(pci_dev, reg
						, (unsigned short*) data);
			}
			break;
		case 4:
			{
				g_pci_read_config_dword(pci_dev, reg
						, (unsigned long*) data);
...
int pci_set_data(struct pci_dev_t *pci_dev
		, unsigned char reg
		, unsigned len
		, unsigned char *data)
{
	switch(len)
	{
		case 1:
			{
				g_pci_write_config_byte(pci_dev, reg, *data);
			}
			break;
		case 2:
			{
				g_pci_write_config_word(pci_dev, reg
						, *((unsigned short*) data));
			}
			break;
		case 4:
			{
				g_pci_write_config_dword(pci_dev, reg
						, *((unsigned long*) data));
...
int pci_set_master(struct pci_dev_t *pci_dev)
{
	unsigned short cmd;

	g_pci_read_config_word(pci_dev, PCI_REG_CMD, &cmd);
	printk("pci_set_master: cmd= 0x%x\n", cmd);
	if (!(cmd & PCI_REG_CMD_MASTER))
	{
		printk("pci_set_master: enabling bus mastering for dev: 0x%x\n"
				, pci_dev->dev_id);
		cmd |= PCI_REG_CMD_MASTER;
		g_pci_write_config_word(pci_dev, PCI_REG_CMD, cmd);
		return(0);
	}
	return(-1);
}
